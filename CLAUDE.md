...
| **Campaign** ||
| `project/scripts/campaign/campaign_data.gd` | `CampaignData` — the four Canarii missions as a const table: fixed seed + MatchConfig fields, victory kind (`conquest`/`regicide`/`survive`), side objectives (`train`/`build`/`destroy`), scripted waves |
