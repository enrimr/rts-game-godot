...
Long-form reference moved out of `CLAUDE.md` (2026-09-15) to keep that file
small: the per-file notes below preserve the full detail (constants, bug
history, design rationale) that used to live in the `CLAUDE.md` Key Files
table, plus the curated CI-gate harness command block and the complete
feature inventory. `CLAUDE.md` keeps a one-line-per-file map; per-system
design lives in [systems.md](systems.md).

When a detail here goes stale, fix it here (or in `systems.md`) — do not
grow it back into `CLAUDE.md`.

## Key files (detailed table)
...
## Testing & CI-gate harness commands
