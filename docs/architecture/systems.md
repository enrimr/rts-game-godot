...
- **BottomBar** (`PanelContainer`, anchored bottom): a selection panel and a minimap panel.
  ...
  - **MinimapPanel**: hosts `MinimapRenderer` (`scripts/ui/minimap.gd`). Rendering is split across two child layers so the widget is not rebuilt every frame: **ContentLayer** (terrain texture, fog cells, resources, buildings, units — the expensive entity iteration) redraws on a decoupled tick every `CONTENT_REDRAW_INTERVAL` (0.2 s), while **OverlayLayer** (camera viewport rect, event flashes, border) is marked dirty only when the camera view actually changes or a flash is animating; an idle minimap issues no draw calls.
